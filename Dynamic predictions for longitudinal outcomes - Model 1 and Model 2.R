...
# generate the linear predictor (eta) in longitudinal count data process by using the conditional approach
eta.con<-function(para1,i1,t1,s1){
  # import sampled parameters
  ...
  fam_hist<-fam_select[(fam_select$dur + t1)>=0,]
  ...
  z.long <- c(rep(NA,n1),data.long.z$num,rep(NA,n3+n4+2))
  y.recu<-inla.surv(time=c(rep(NA,n1+n2),T.R.f/max(T.R.f),rep(NA,n4)), event=c(rep(NA,n1+n2),Delta.R.f,rep(NA,n4+2)))
  y.term<-inla.surv(time=c(rep(NA,n1+n2+n3),T.fh.D/max(T.fh.D)), event=c(rep(NA,n1+n2+n3),delta.fh.D,rep(NA,2)))
  ...
  linear.covariate <- data.frame(mu = as.factor(c(rep(NA,n1+n2),rep(1,n3),rep(2,n4),rep(NA,2))),  # exp(-mu1) and exp(-mu2) = scale parameters in weibull distribution for recurrent events and terminal event respectively
                                 l.Time = c(log(data.long$gap), rep(NA,n2+n3+n4+2)), # related to logit(pi): logistic component for longitudinal outcomes 
                                 l.Sex = c(data.long$sex01, rep(NA,n2+n3+n4),data$sex01[data$id==i1][1],rep(NA,1)),
                                 l.Age = c(data.long$age00_trans, rep(NA,n2+n3+n4),data$age00_trans[data$id==i1][1],rep(NA,1)),
                                 l.Pcrc1age = c(data.long$pcrc1age_trans, rep(NA,n2+n3+n4),data$pcrc1age_trans[data$id==i1][1],rep(NA,1)),
                                 l.Cumgap = c(data.long$cumgap0, rep(NA,n2+n3+n4),if(t1==0){0} else{data$cumgap0[data$id==i1 & data$cumgap>=t1][1]},rep(NA,1)),
                                 z.Time = c(rep(NA,n1), log(data.long.z$gap), rep(NA,n3+n4+2)), # related to log(mu): negative binomial component for longitudinal outcomes
                                 z.Sex = c(rep(NA,n1), data.long.z$sex01, rep(NA,n3+n4+1),data$sex01[data$id==i1][1]),
                                 z.Age = c(rep(NA,n1), data.long.z$age00_trans, rep(NA,n3+n4+1),data$age00_trans[data$id==i1][1]),
                                 z.Pcrc1age = c(rep(NA,n1), data.long.z$pcrc1age_trans, rep(NA,n3+n4+1),data$pcrc1age_trans[data$id==i1][1]),
                                 z.Cumgap = c(rep(NA,n1), data.long.z$cumgap0, rep(NA,n3+n4+1),if(t1==0){0} else{data$cumgap0[data$id==i1 & data$cumgap>=t1][1]}),   
                                 r.Sex = c(rep(NA,n1+n2), data.rec$sex01, rep(NA,n4+2)), # relative to hazard function for recurrent events
                                 r.Adenoma = c(rep(NA,n1+n2), data.rec$adenoma0, rep(NA,n4+2)),
                                 r.Age = c(rep(NA,n1+n2), data.rec$age0_trans, rep(NA,n4+2)),
                                 r.Pcrc1age = c(rep(NA,n1+n2), data.rec$pcrc1age_trans, rep(NA,n4+2)),
                                 d.Sex = c(rep(NA,n1+n2+n3), data.term$sex01,rep(NA,2)), # related to hazard function for terminal event
                                 d.Adenoma = c(rep(NA,n1+n2+n3), data.term$adenoma00,rep(NA,2)),
                                 d.Age = c(rep(NA,n1+n2+n3), data.term$age00_trans,rep(NA,2)),
                                 d.Pcrc1age = c(rep(NA,n1+n2+n3), data.term$pcrc1age_trans,rep(NA,2)))
  
  random.covariate <- list(l.YL = c(data.long$id+177, rep(NA,n2+n3+n4),i1+177,rep(NA,1)), 
                           l.ZL = c(data.long$famid+18, rep(NA,n2+n3+n4),data$famid[data$id==i1][1]+18,rep(NA,1)), 
                           z.YL = c(rep(NA,n1), data.long.z$id+177, rep(NA,n3+n4+1),i1+177), 
                           z.ZL = c(rep(NA,n1), data.long.z$famid+18, rep(NA,n3+n4+1),data$famid[data$id==i1][1]+18), 
                           r.YR = c(rep(NA,n1+n2), data.rec$id, rep(NA,n4+2)), 
                           r.ZR = c(rep(NA,n1+n2), data.rec$famid, rep(NA,n4+2)), 
                           d.YR = c(rep(NA,n1+n2+n3), data.term$id,rep(NA,2)), 
                           d.ZR = c(rep(NA,n1+n2+n3), data.term$famid,rep(NA,2)), 
                           d.YL = c(rep(NA,n1+n2+n3), data.term$id+177,rep(NA,2)), 
                           d.ZL = c(rep(NA,n1+n2+n3), data.term$famid+18,rep(NA,2)))
  ...
  # model 1
  eta.lc1<-model1$summary.linear.predictor$mean[n1+n2+n3+n4+1]+chi0+log(s1)
  pi.c1<-exp(eta.lc1)/(1+exp(eta.lc1))
  eta.zc1<-model1$summary.linear.predictor$mean[n1+n2+n3+n4+2]+theta0+log(s1)
  mu.c1<-exp(eta.zc1)
  dy.c1<-(1-pi.c1)*mu.c1
  
  # random effects
  l.ZL.1<-model1$summary.random$r.ZR$mean[famid_select+18]
  
  # model 2
  eta.lc2<-model1$summary.linear.predictor$mean[n1+n2+n3+n4+1]+chi0+log(s1)-tau.l*l.ZL.1
  pi.c2<-exp(eta.lc2)/(1+exp(eta.lc2))
  eta.zc2<-model1$summary.linear.predictor$mean[n1+n2+n3+n4+2]+theta0+log(s1)-tau.z*l.ZL.1
  mu.c2<-exp(eta.zc2)
  dy.c2<-(1-pi.c2)*mu.c2
...
# for t = 0
data.long2<-data[!is.na(data$num),]
n1<-nrow(data.long2)
m1<-5
dp.i.f.1c<-dp.i.f.2c<-cbind(data.long2$id,rep(-1,n1),data.long2$num)
for (i1 in 1:n1){
  i2<-data.long2$id[i1]
  t2<-if (data.long2$time[i1]==2) {0} else {data.long2$cumgap[i1]-data.long2$gap[i1]}
  s2<-data.long2$gap[i1]
  result.ind<-result.ind2<-rep(-1,m1)
  
  for (j in 1:m1){
    # result
    result<-eta.con(param_sample[j,],i2,t2,s2)
    
    ## model 1
    result.ind[j]<-result[1]
    
    ## model 2
    result.ind2[j]<-result[2]
    
    print(c(i2,j))
  }
  
  dp.i.f.1c[i1,2]<-median(result.ind,na.rm=TRUE)
  dp.i.f.2c[i1,2]<-median(result.ind2,na.rm=TRUE)
  
  write.csv(dp.i.f.1c,"dp-lo-1c-0.csv",row.names=FALSE)
  write.csv(dp.i.f.2c,"dp-lo-2c-0.csv",row.names=FALSE)
}
