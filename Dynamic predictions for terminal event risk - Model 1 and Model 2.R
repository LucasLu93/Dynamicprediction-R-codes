...
eta.con<-function(para1,i1,t1){
  ...
  # model 1
  eta.c1<-model1$summary.linear.predictor$mean[n1+n2+n3+which(id.D.f==i1)]

  # model 2
  eta.c2<-model1$summary.linear.predictor$mean[n1+n2+n3+which(id.D.f==i1)]-
    xi*model1$summary.random$d.ZL$mean[famid_select]-model1$summary.random$d.ZL$mean[famid_select+18]
  
  return(c(eta.c1,eta.c2))
}
...
n2<-length(ind)
m2<-100
dp.i.f.1c<-dp.i.f.2c<-matrix(-1,nrow=n2,ncol=15)
for (i2 in 1:n2){
  i<-ind[i2]
  result.ind<-result.ind2<-matrix(-1,nrow=m2,ncol=15)
  ...
  for (j in 1:m2){
    ...
    # model 1
    eta.c1<-result[1]
    result.ind[j,s]<-1-exp(-((t2+s)/b.D)^a.D*exp(eta.c1) + (t2/b.D)^a.D*exp(eta.c1))
    
    # model 2
    eta.c2<-result[2]
    result.ind2[j,s]<-1-exp(-((t2+s)/b.D)^a.D*exp(eta.c2) + (t2/b.D)^a.D*exp(eta.c2))
  }
  
  dp.i.f.1c[i2,]<-apply(result.ind,2,median,na.rm=TRUE)
  dp.i.f.2c[i2,]<-apply(result.ind2,2,median,na.rm=TRUE)
  
  write.csv(cbind(ind,dp.i.f.1c),"dp-te-1c-0.csv",row.names=FALSE)
  write.csv(cbind(ind,dp.i.f.2c),"dp-te-2c-0.csv",row.names=FALSE)
}
