...
eta.con<-function(para1,i1,t1,s1){
  ...
  # model 3
  eta.lc3<-model1$summary.linear.predictor$mean[n1+n2+n3+n4+1]+chi0+log(s1)
  pi.c3<-exp(eta.lc3)/(1+exp(eta.lc3))
  eta.zc3<-model1$summary.linear.predictor$mean[n1+n2+n3+n4+2]+theta0+log(s1)
  mu.c3<-exp(eta.zc3)
  dy.c3<-(1-pi.c3)*mu.c3
...
for (i1 in 1:n1){
  ...
  for (j in 1:m1){
    ...
    ## model 3
    result.ind3[j]<-result
    
    print(c(i2,j))
  }
  
  dp.i.f.3c[i1,2]<-median(result.ind3,na.rm=TRUE)
  
  write.csv(dp.i.f.3c,"dp-lo-3c-0.csv",row.names=FALSE)
}
